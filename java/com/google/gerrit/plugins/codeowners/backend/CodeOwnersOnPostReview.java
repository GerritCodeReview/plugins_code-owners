...
class CodeOwnersOnPostReview implements OnPostReview {
  ...
  public Optional<String> getChangeMessageAddOn(
    ...
    return buildMessageForCodeOwnerApproval(
        user, changeNotes, oldApprovals, approvals, requiredApproval);
  }
  ...
  private Optional<String> buildMessageForCodeOwnerApproval(
    ...
    if (isCodeOwnerApprovalNewlyApplied(requiredApproval, oldApprovals, newVote)) {
      message.append(
          String.format(
              "%s is a code owner." + " By voting %s the following paths are now approved by %s:\n",
              user.getName(), newVote, user.getName()));
    } else if (isCodeOwnerApprovalRemoved(requiredApproval, oldApprovals, newVote)) {
      if (newVote.value() == 0) {
        message.append(
            String.format(
                "%s is a code owner."
                    + " By removing the %s vote the following paths are no longer approved by"
                    + " %s:\n",
                user.getName(), newVote.label(), user.getName()));
      } else {
        message.append(
            String.format(
                "%s is a code owner."
                    + " By voting %s the following paths are no longer approved by %s:\n",
                user.getName(), newVote, user.getName()));
      }
      ...
      message.append(
          String.format(
              "%s is a code owner."
                  + " By voting %s the following paths are still approved by %s:\n",
              user.getName(), newVote, user.getName()));
    } else {
      return Optional.empty();
    }
