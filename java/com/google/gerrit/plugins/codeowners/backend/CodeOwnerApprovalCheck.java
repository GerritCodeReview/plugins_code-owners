...
public class CodeOwnerApprovalCheck {
   ...
   */
  public ImmutableSet<FileCodeOwnerStatus> getStatuses(RevisionResource revisionResource)
      throws IOException {
    ...
    for (ChangedFile changedFile : changedFiles.compute(revisionResource)) {
      ...
      Optional<PathCodeOwnerStatus> newPathCodeOwnerStatus =
          changedFile
              ...
              .map(
                  newPath ->
                      getCodeOwnerStatus(branch, reviewerAccountIds, approverAccountIds, newPath));

      // Compute the code owner status for the old path, if the file was deleted or renamed.
      Optional<PathCodeOwnerStatus> oldPathCodeOwnerStatus = Optional.empty();
      ...
      fileCodeOwnerStatusBuilder.add(
          FileCodeOwnerStatus.create(changedFile, newPathCodeOwnerStatus, oldPathCodeOwnerStatus));
    }
  ...
  private PathCodeOwnerStatus getCodeOwnerStatus(
    ...
    codeOwnerConfigHierarchy.visit(
        ...
        codeOwnerConfig -> {
          ...
          // We can abort since we already found that the file was approved.
          return false;
        });
