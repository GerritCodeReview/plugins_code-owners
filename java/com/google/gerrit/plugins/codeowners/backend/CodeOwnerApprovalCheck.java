...
public class CodeOwnerApprovalCheck {
  ...
  public ImmutableList<OwnedChangedFile> getOwnedPaths(
      ChangeNotes changeNotes, PatchSet patchSet, Account.Id accountId, int start, int limit, boolean checkReviewers) {
    try (Timer0.Context ctx = codeOwnerMetrics.computeOwnedPaths.start()) {
      logger.atFine().log(
          ...
          start,
          limit);
      ImmutableSet<Account.Id> reviewerAccountIds = ImmutableSet.of();
      if (checkReviewers) {
        reviewerAccountIds = changeNotes.getReviewers().byState(ReviewerStateInternal.REVIEWER);
      }
      Stream<FileCodeOwnerStatus> fileStatuses =
          getFileStatusesForAccount(changeNotes, patchSet, accountId, reviewerAccountIds)
              .filter(
                  fileStatus ->
   ...
   * given account.
   *
   * <p> As a side-effect, it also computes, for each file, who the approvers are if the
   ...
   * @param reviewerAccountIdsd The reviewers for the given patchset.
   */
  @VisibleForTesting
  ...
  private PathCodeOwnerStatus getPathCodeOwnerStatus(
    ...
    ImmutableSet.Builder reviewerOwners = ImmutableSet.builder();
    ...
    PathCodeOwnerStatus pathCodeOwnerStatus =
        PathCodeOwnerStatus.create(absolutePath, codeOwnerStatus.get(), reason.get(), reviewerOwners.build());
