...
public class CodeOwnerApprovalCheck {
  ...
  private boolean isImplicitlyApprovedBootstrappingMode(
    ...
    if (isProjectOwner(projectName, patchSetUploader)) {
      ...
      logger.atFine().log(
          ...
          absolutePath);
      return true;
    } else if (globalCodeOwners.ownedByAllUsers()
  ...
  private boolean isPendingBootstrappingMode(
    ...
    if (reviewerAccountIds.stream()
      ...
      return true;
    } else if (isPending(absolutePath, globalCodeOwners, reviewerAccountIds)) {
  ...
  private boolean isApproved(
    ...
    if (enableImplicitApprovalFromUploader
      ...
      return true;
    } else if (!Collections.disjoint(approverAccountIds, codeOwners.codeOwnersAccountIds())
