    ...
    while (!codeOwnerConfigsToImport.isEmpty()) {
      ...
      if (importMode.resolveImportsOfImport()
        ...
        if (importMode == CodeOwnerConfigImportMode.GLOBAL_CODE_OWNER_SETS_ONLY) {
          // If only global code owners should be imported, also for transitive imports only global
          // code owners should be imported, no matter which import mode is specified in the
          // imported code owner configs.
          transitiveImports =
