...
public class FileCodeOwnerStatusSubject extends Subject {
  ...
  public OptionalSubject<Subject, ?> hasNewPathCodeOwnerStatus() {
    return check("newPathCodeOwnerStatus()")
        .about(optionals())
        .that(fileCodeOwnerStatus().newPathCodeOwnerStatus());
  }

  /** Returns an {@link OptionalSubject} for the code owners status of the old path. */
  public OptionalSubject<Subject, ?> hasOldPathCodeOwnerStatus() {
    return check("oldPathCodeOwnerStatus()")
        .about(optionals())
        .that(fileCodeOwnerStatus().oldPathCodeOwnerStatus());
  }
