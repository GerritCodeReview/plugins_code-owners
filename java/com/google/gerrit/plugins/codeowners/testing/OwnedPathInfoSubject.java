...
public class OwnedPathInfoSubject extends Subject {
  ...
  private OwnedPathInfoSubject(FailureMetadata metadata, OwnedPathInfo ownedPathInfo) {
  ...
  public IterableSubject hasOwnersThat() {
    return check("owners.get").that(ownedPathInfo().owners.get());
  }

  public void hasOwners(TestAccount... testAccounts) {
    ImmutableList.Builder<AccountInfo> accounts = ImmutableList.builder();
    for (TestAccount testAccount : testAccounts) {
      accounts.add(new AccountInfo(testAccount.id().get()));
    }
