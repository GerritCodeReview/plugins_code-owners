 ...
 *
 * <p>This class also defines {@link ConfigSuite}s to execute each integration test for all code
 * owner backends.
 */
public class AbstractCodeOwnersIT extends AbstractCodeOwnersTest {
  ...
  public static Map<String, Config> againstCodeOwnerBackends() {
    return Stream.of(CodeOwnersBackends.values())
        .collect(
            toImmutableMap(
                CodeOwnersBackends::getBackendId,
                AbstractCodeOwnersIT::createConfigWithCodeOwnersBackend));
  }

  private static Config createConfigWithCodeOwnersBackend(CodeOwnersBackends codeOwnersBackend) {
    Config cfg = new Config(defaultConfig());
    cfg.setString(
        "plugin",
        "code-owners",
        CodeOwnersPluginConfiguration.KEY_BACKEND,
        codeOwnersBackend.getBackendId());
    return cfg;
  }
