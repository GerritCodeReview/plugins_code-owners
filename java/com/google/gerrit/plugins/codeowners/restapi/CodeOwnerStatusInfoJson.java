...
public class CodeOwnerStatusInfoJson {
  ...
  private static final Comparator<FileCodeOwnerStatus> FILE_CODE_OWNER_STATUS_COMPARATOR =
      comparing(
          (FileCodeOwnerStatus a) -> {
            if (a.newPathCodeOwnerStatus().isPresent()) {
              return a.newPathCodeOwnerStatus().get().path().toString();
            }
            if (a.oldPathCodeOwnerStatus().isPresent()) {
              return a.oldPathCodeOwnerStatus().get().path().toString();
            }
            // should never happen since one of new path and old path is always set
            return "";
          });
  ...
  static FileCodeOwnerStatusInfo format(FileCodeOwnerStatus fileCodeOwnerStatus) {
    ...
    fileCodeOwnerStatus
        ...
        .ifPresent(
            oldPathCodeOwnerStatus -> info.oldPathCodeOwnerStatus = format(oldPathCodeOwnerStatus));
    fileCodeOwnerStatus
        .newPathCodeOwnerStatus()
        .ifPresent(
            newPathCodeOwnerStatus -> info.newPathCodeOwnerStatus = format(newPathCodeOwnerStatus));
    return info;
  }
  ...
  static PathCodeOwnerStatusInfo format(PathCodeOwnerStatus pathCodeOwnerStatus) {
    ...
    info.codeOwnerStatus = pathCodeOwnerStatus.codeOwnerStatus();
    return info;
  }
