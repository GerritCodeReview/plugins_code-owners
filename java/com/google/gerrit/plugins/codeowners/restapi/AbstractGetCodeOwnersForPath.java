...
public abstract class AbstractGetCodeOwnersForPath<R extends AbstractPathResource>
  ...
  protected Response<CodeOwnersInfo> applyImpl(R rsrc)
    ...
    ImmutableList<CodeOwnerScoringFactorInfo> codeOwnersScoringFactors = getCodeOwnersScoringFactors(
    ...
    codeOwnersInfo.codeOwnersScoringFactors = codeOwnersScoringFactors;
  ...
  private ImmutableList<CodeOwnerScoringFactorInfo> getCodeOwnersScoringFactors(
    ...
    List<CodeOwnerScoringFactorInfo> codeOwnerScoringFactorInfos = new ArrayList<>();
    for(CodeOwner codeOwner : codeOwnersList) {
      ...
      if(codeOwnerInfo != null) {
        ...
        if(codeOwnerScoringFactors != null) {
          for(Map.Entry<CodeOwnerScore, Integer> entry : codeOwnerScoringFactors.getScoringFactors().entrySet()){
            CodeOwnerScoringFactorInfo scoringFactorInfo = new CodeOwnerScoringFactorInfo();
            scoringFactorInfo.codeOwnerInfo = codeOwnerInfo;
            scoringFactorInfo.score = entry.getKey().name();
            scoringFactorInfo.value = entry.getValue().toString();
            codeOwnerScoringFactorInfos.add(scoringFactorInfo);
          }
        }
      }
    }
    return ImmutableList.copyOf(codeOwnerScoringFactorInfos);
  }
