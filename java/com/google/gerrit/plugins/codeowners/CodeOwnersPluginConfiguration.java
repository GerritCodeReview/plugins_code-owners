...
public class CodeOwnersPluginConfiguration {
  ...
  public CodeOwnersBackend getBackend(Project.NameKey project) {
  	logger.atInfo().log("[ekempin] getting backend for project %s", project);
    Config pluginConfig = getPluginConfig(project);
    logger.atInfo().log("[ekempin] plugin config: %s", pluginConfig.toText());
    String backendName = pluginConfig.getString(SECTION_CODE_OWNERS, null, KEY_BACKEND);
    logger.atInfo().log("[ekempin] backend from plugin config: %s", backendName);
    if (backendName == null) {
  ...
  private CodeOwnersBackend getDefaultBackend() {
    if (defaultCodeOwnersBackend == null) {
      logger.atInfo().log("[ekempin] reading default backend for plugin %s", pluginName);
      String backendName =
          pluginConfigFactory
              .getFromGerritConfig(pluginName)
              .getString(KEY_BACKEND, FindOwnersBackend.ID);
      logger.atInfo().log("[ekempin] default backend: %s", backendName);
  ...
  private Config getPluginConfig(Project.NameKey project) {
    logger.atInfo().log("[ekempin] reading plugin config for plugin %s and project %s", pluginName, project);
    try {
  ...
  private Optional<CodeOwnersBackend> lookupBackend(String backendName) {
    ...
    // plugin name.
  	CodeOwnersBackend b = codeOwnersBackends.get("gerrit", backendName);
  	logger.atInfo().log("[ekempin] looked-up backend: %s", b != null ? b.getClass().getName() : null);
