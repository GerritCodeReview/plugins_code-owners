...
public class CodeOwnersPluginConfiguration {
  ...
  public CodeOwnersBackend getBackend(Project.NameKey project) {
    ...
    logger.atInfo().log("backend for project %s: %s", project, backendName);
    if (backendName == null) {
  ...
  private CodeOwnersBackend getDefaultBackend() {
    if (defaultCodeOwnersBackend == null) {
      logger.atInfo().log("reading default backend for plugin %s", pluginName);
      String backendName =
          pluginConfigFactory
              .getFromGerritConfig(pluginName)
              .getString(KEY_BACKEND, FindOwnersBackend.ID);
      logger.atInfo().log("default backend: %s", backendName);
  ...
  private Config getPluginConfig(Project.NameKey project) {
    logger.atInfo().log("reading plugin config for plugin %s and project %s", pluginName, project);
    try {
