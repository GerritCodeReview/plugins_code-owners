...
public class CodeOwnerHasOperandsIT extends AbstractCodeOwnersIT {
  ...
  public void setup() throws Exception {
    ...
    configSubmitRequirement(
        project,
        SubmitRequirement.builder()
            .setName("Code-Owner-Enabled")
            .setSubmittabilityExpression(
                SubmitRequirementExpression.create("has:enabled_code-owners"))
            .setAllowOverrideInChildProjects(false)
            .build());
  }
  ...
  public void hasApproval_ruleErrorForNonParsableCodeOwnerConfig() throws Exception {
    ...
    // Requirement is unsatisfied if an internal error happens while evaluating the SR.
    assertSubmitRequirement(changeInfo.submitRequirements, "Code-Owner-Approval", UNSATISFIED);
  }
  ...
  public void hasEnabled_matchingWhenCodeOwnersIsEnabledForTheChange() throws Exception {
    ...
    ChangeInfo changeInfo = gApi.changes().id(changeId).get(ListChangesOption.SUBMIT_REQUIREMENTS);
    assertSubmitRequirement(changeInfo.submitRequirements, "Code-Owner-Enabled", SATISFIED);
  ...
  public void hasEnabled_notMatchingWhenCodeOwnersIsDisabledForTheChange() throws Exception {
    ...
    String changeId = change.getKey().get();
    ChangeInfo changeInfo = gApi.changes().id(changeId).get(ListChangesOption.SUBMIT_REQUIREMENTS);
    assertSubmitRequirement(changeInfo.submitRequirements, "Code-Owner-Enabled", UNSATISFIED);
