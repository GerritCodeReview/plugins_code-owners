...
public class GetCodeOwnersForPathInBranchIT extends AbstractCodeOwnersIT {
  ...
  public void getPerFileCodeOwners() throws Exception {
    ...
    CodeOwners codeOwnersApi = codeOwnersApiFactory.branch(project, "master");
    assertThat(codeOwnersApi.query().get("/foo/bar/config.txt"))
        .comparingElementsUsing(hasAccountId())
        .containsExactly(admin.id());
    assertThat(codeOwnersApi.query().get("/foo/bar/baz.md"))
        .comparingElementsUsing(hasAccountId())
        .containsExactly(user.id());
    assertThat(codeOwnersApi.query().get("/foo/bar/main.config")).isEmpty();
  }
