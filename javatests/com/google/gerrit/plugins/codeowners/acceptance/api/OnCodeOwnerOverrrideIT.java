...
public class OnCodeOwnerOverrrideIT extends AbstractCodeOwnersIT {
  ...
  public void changeMessageNotExtendedIfNonCodeOwnersOverrideIsApplied() throws Exception {
    ...
    gApi.projects().name(project.get()).label("Other-Label").create(input).get();
    ...
    projectOperations
        ...
        .forUpdate()
        .add(
            TestProjectUpdate.allowLabel("Other-Label")
                .range(0, 1)
                .ref("refs/*")
                .group(REGISTERED_USERS)
                .build())
    ...
    gApi.changes().id(changeId).current().review(new ReviewInput().label("Other-Label", 1));

    Collection<ChangeMessageInfo> messages = gApi.changes().id(changeId).get().messages;
    assertThat(Iterables.getLast(messages).message).isEqualTo("Patch Set 1: Other-Label+1");
  }
  ...
  public void changeMessageExtendedIfMultipleCodeOwnersOverridesAreAppliedTogether()
    ...
    assertThat(Iterables.getLast(messages).message)
        .isEqualTo(
            String.format(
                "Patch Set 1: Owners-Override+1 Global-Override+1\n\n"
                    + "By voting Owners-Override+1 the code-owners submit requirement is"
                    + " overridden by %s\n\n"
  ...
  public void changeMessageExtendedIfCodeOwnersOverrideAndCodeOwnerApprovalAreAppliedTogether()
    ...
    Collection<ChangeMessageInfo> messages = gApi.changes().id(changeId).get().messages;
    assertThat(Iterables.getLast(messages).message)
        .startsWith("Patch Set 1: Owners-Override+1 Code-Review+1\n\n");
    assertThat(Iterables.getLast(messages).message)
        .contains(
            String.format(
                "By voting Owners-Override+1 the code-owners submit requirement is"
