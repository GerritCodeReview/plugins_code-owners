...
public class CodeOwnersPluginConfigurationTest extends AbstractCodeOwnersTest {
  ...
  public void cannotGetRequiredApprovalIfNonExistingLabelIsConfiguredAsRequiredApproval()
    ...
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Required approval 'Foo-Bar+1' that is configured in gerrit.config"
                    + " (parameter plugin.code-owners.requiredApproval) is invalid:"
                    + " label Foo-Bar doesn't exist for project %s",
                project.get()));
  }
  ...
  public void cannotGetRequiredApprovalIfNonExistingLabelValueIsConfiguredAsRequiredApproval()
    ...
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Required approval 'Code-Review+3' that is configured in gerrit.config"
                    + " (parameter plugin.code-owners.requiredApproval) is invalid:"
                    + " label Code-Review on project %s doesn't allow value 3",
                project.get()));
  }
  ...
  public void cannotGetRequiredApprovalIfInvalidRequiredApprovalIsConfigured() throws Exception {
    ...
    assertThat(exception)
        .hasMessageThat()
        .isEqualTo(
            "Required approval 'INVALID' that is configured in gerrit.config"
                + " (parameter plugin.code-owners.requiredApproval) is invalid:"
  ...
  public void
    ...
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Required approval 'Foo-Bar+1' that is configured in code-owners.config"
                    + " (parameter codeOwners.requiredApproval) is invalid:"
                    + " label Foo-Bar doesn't exist for project %s",
                project.get()));
  }
  ...
  public void
    ...
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Required approval 'Code-Review+3' that is configured in code-owners.config"
                    + " (parameter codeOwners.requiredApproval) is invalid:"
                    + " label Code-Review on project %s doesn't allow value 3",
                project.get()));
  }
  ...
  public void cannotGetRequiredApprovalIfInvalidRequiredApprovalIsConfiguredOnProjectLevel()
    ...
    assertThat(exception)
        .hasMessageThat()
        .isEqualTo(
            "Required approval 'INVALID' that is configured in code-owners.config"
                + " (parameter codeOwners.requiredApproval) is invalid:"
                + " invalid format, expected '<label-name>+<label-value>'");
  }
