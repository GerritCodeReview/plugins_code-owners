...
public class CodeOwnersPluginProjectConfigSnapshotTest extends AbstractCodeOwnersTest {
  ...
  @Test
  public void cannotGetPathExpressionsIfNonExistingPathExpressionsAreConfiguredOnProjectLevel()
      throws Exception {
    ...
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Invalid configuration of the code-owners plugin. Path expressions"
                    + " 'non-existing-path-expressions' that are configured for project %s in"
                    + " code-owners.config (parameter codeOwners.pathExpressions) not found.",
                project));
  }
  ...
  @Test
  public void cannotGetPathExpressionsIfNonExistingPathExpressionsAreConfiguredOnBranchLevel()
      throws Exception {
    configurePathExpressions(project, "master", "non-existing-path-expressions");
    InvalidPluginConfigurationException exception =
        assertThrows(
            InvalidPluginConfigurationException.class,
            () -> cfgSnapshot().getPathExpressions("master"));
    assertThat(exception)
        ...
        .isEqualTo(
            String.format(
                "Invalid configuration of the code-owners plugin. Path expressions"
                    + " 'non-existing-path-expressions' that are configured for project %s in"
                    + " code-owners.config (parameter codeOwners.master.pathExpressions) not found.",
                project));
  }
