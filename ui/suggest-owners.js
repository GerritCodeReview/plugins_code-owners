...
export class SuggestOwners extends Polymer.Element {
  ...
  static get template() {
    return Polymer.html`
      ...
      <div hidden="[[hasMoreFetched]]">
        <p class="loading" hidden="[[!isFetchingMore]]">
          <span class="loadingSpin"></span>
          loading...
        </p>
        <span hidden="[[isFetchingMore]]" on-click="getOwnersOnApprovedFiles">
          Approved files not included, click to fetch all.
        </span>
      </div>
    `;
  }
  ...
  onInputChanged(restApi, change) {
    ...
    this.ownerService.getSuggestedOwners({skipApproved: true})
    .then((suggestedOwners) => {
      this.isLoading = false;
      this.suggestedOwners = suggestedOwners.map((suggestion) => {
        return this.formatSuggestionInfo(suggestion);
      }).sort((groupA, groupB) => {
        // Not fullfiled should show at the top
        return 0;
      });
    });
  }

  getOwnersOnApprovedFiles() {
    this.isFetchingMore = true;
    this.ownerService.getSuggestedOwners({onlyApproved: true})
    .then((suggestedOwners) => {
      this.isFetchingMore = false;
      this.hasMoreFetched = true;
      this.suggestedOwners = this.suggestedOwners.concat(suggestedOwners.map((suggestion) => {
        return this.formatSuggestionInfo(suggestion);
      }));
    });
  }
