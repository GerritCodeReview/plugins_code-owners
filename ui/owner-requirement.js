...
export class OwnerRequirementValue extends Polymer.Element {
  ...
  _updateStatus(ownerService) {
    this.isLoading = true;
    return ownerService.getStatus().then(({file_owner_statuses}) => {
      let statusText = '';
      const statusCount = Object.keys(file_owner_statuses)
          .reduce((prev, cur) => {
            if (
              file_owner_statuses[cur] === OwnerStatus.INSUFFICIENT_REVIEWERS
            ) {
              prev.missing ++;
            } else if (file_owner_statuses[cur] === OwnerStatus.PENDING) {
              prev.pending ++;
            }
            return prev;
          }, {missing: 0, pending: 0});
      ...
      if (statusCount.missing) {
        statusText = `${statusCount.missing} file(s) missing, `;
      }

      if (statusCount.pending) {
        statusText = `${statusCount.pending} file(s) pending`;
      }
