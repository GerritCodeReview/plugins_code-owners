 ...
 */
export class OwnerRequirementValue extends CodeOwnersMixin(Polymer.Element) {
  static get is() {
  ...
  static get template() {
    return Polymer.html`
        ...
        <template is="dom-if" if="[[!_isLoading]]">
          <template is="dom-if" if="[[!_pluginFailed(ownersState.pluginState)]]">        
    ...
    return {
      ...
      _isLoading: {
        ...
        computed: '_computeIsLoading(ownersState.branchConfig, ' +
            'ownersState.status, ownersState.pluginState, ' +
            'ownersState.userRole)',
      },
      _allApproved: {
        type: Boolean,
  ...
  static get observers() {
    return [
      ...
      '_onStatusChanged(ownersState.status, ownersState.userRole)',
    ];
  }

  _computeIsLoading(branchConfig, status, pluginState, userRole) {
  ...
  _onStatusChanged(status, userRole) {
    ...
    this.reporting.reportLifeCycle('owners-submit-requirement-summary-shown', {...this._statusCount, user_role: userRole});
  }
  ...
  _openReplyDialog() {
    ...
    this.reporting.reportInteraction(
        'suggest-owners-from-submit-requirement', {user_role: this.ownersState.userRole});
  }

  _showFailDetails() {
    this.dispatchEvent(
        new CustomEvent('show-error', {
          detail: {message: this.ownersState.failedReason},
          composed: true,
          bubbles: true,
        })
    );
