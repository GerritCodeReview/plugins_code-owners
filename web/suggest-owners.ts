...
export class SuggestOwners extends base {
  ...
  private currentSuggestionsType?: SuggestionsType;
  ...
  static override get styles() {
    return [
      // TODO: shared-styles
      // window.Gerrit.styles.icon as CSSResult,
      css`
        :host {
          display: block;
  ...
  protected override willUpdate(changedProperties: PropertyValues): void {
    ...
    if (changedProperties.has('model')) {
      // NOTE: This overtriggers as we do not know which part changed.
      this.onShowSuggestionsChanged();
      if (this.currentSuggestionsType !== this.model?.selectedSuggestionsType) {
        this.currentSuggestionsType = this.model?.selectedSuggestionsType;
        this.onShowSuggestionsTypeChanged();
      }
      this.onSuggestionsStateChanged();
    }
    if (changedProperties.has('model') || changedProperties.has('reviewers')) {
  ...
  private renderLoading() {
    const isLoading =
      this.model?.selectedSuggestions?.state === SuggestionsState.Loading;
    if (!isLoading) return nothing;
    return html`
      <p class="loading">
        <span class="loadingSpin"></span>
        ${this.model?.selectedSuggestions?.loadProgress}
      </p>
    `;
  }
  ...
  private onShowSuggestionsTypeChanged() {
    ...
    const selectedSuggestionsType = this.model?.selectedSuggestionsType;
  ...
  private onSuggestionsStateChanged() {
    const state = this.model?.selectedSuggestions?.state;
    this.stopUpdateProgressTimer();
  ...
  private isHidden() {
    return !this.model?.showSuggestions;
  }
  ...
  private onSuggestionsFilesChanged() {
    const allOwners = this.model?.suggestionsByTypes.get(
      SuggestionsType.ALL_SUGGESTIONS
    )?.files;

    if (
      this.model?.selectedSuggestions?.files === undefined ||
      allOwners === undefined ||
      this.reviewers === undefined ||
      this.model?.selectedSuggestionsType === undefined ||
      this.model?.selectedSuggestions?.state === undefined
    )
      return;
    ...
    const selectedSuggestionsType = this.model.selectedSuggestionsType;
    const suggestionsState = this.model.selectedSuggestions.state;

    const groups = getDisplayOwnersGroups(
      this.model.selectedSuggestions.files,
      allOwnersByPathMap,
      reviewersIdSet,
  ...
  private getOwnedByAllUsersContent() {
    if (this.model?.selectedSuggestions?.state === SuggestionsState.Loading) {
