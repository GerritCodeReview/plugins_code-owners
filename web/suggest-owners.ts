...
export class SuggestOwners extends base {
  ...
  protected override willUpdate(changedProperties: PropertyValues): void {
    ...
    if (changedProperties.has('suggestedSelectionsType')) {
      this.onShowSuggestionsTypeChanged();
    }
    if (changedProperties.has('suggestedSelectionsState')) {
  ...
  private onSuggestionsStateChanged() {
    const state = this.selectedSuggestionsState;
    this.stopUpdateProgressTimer();
  ...
  private onSuggestionsFilesChanged() {
    const allOwners = this.model?.getSuggestionsByType(
