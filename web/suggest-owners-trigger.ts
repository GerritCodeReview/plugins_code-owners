...
export class SuggestOwnersTrigger extends base {
  ...
  toggleControlContent() {
    ...
    this.reporting?.reportInteraction('toggle-suggest-owners', {
      expanded: this.model?.showSuggestions,
      user_role: this.model?.userRole ? this.model.userRole : 'UNKNOWN',
    });
  }

  computeButtonText() {
    return this.model?.showSuggestions ? 'Hide owners' : 'Suggest owners';
  }
