...
export class OwnerRequirementValue extends base {
  ...
  override render() {
    ...
    if (this.model && this.pluginFailed()) {
      return html`
    ...
    if (this.model.branchConfig?.no_code_owners_defined) {
      // TODO: Remove? no_code_owners_defined is not served from the backend.
      return html`
        <span>No code-owners file</span>
        <a
          href="https://gerrit.googlesource.com/plugins/code-owners/+/HEAD/resources/Documentation/user-guide.md#how-to-submit-changes-with-files-that-have-no-code-owners"
          target="_blank"
        >
          <gr-icon
            icon="help"
            title="Documentation about submitting changes with files that have no code owners?"
          ></gr-icon>
        </a>
      `;
    }
    if (this.model.status?.newerPatchsetUploaded) {
    ...
    return html`
      ...
      ${when(
        this.computeIsSignedInUser(this.model.userRole),
        () => html`
  ...
  private pluginFailed() {
    return (
      this.model?.pluginStatus &&
      isPluginErrorState(this.model?.pluginStatus.state)
    );
  }

  private computeOverrideInfoUrl() {
    if (!this.model?.branchConfig) {
      return '';
    }
    return this.model.branchConfig.general &&
      this.model.branchConfig.general.override_info_url
      ? this.model.branchConfig.general.override_info_url
      : '';
  }
  ...
  private computeIsOverriden() {
    if (
      !this.change ||
      !this.model?.branchConfig ||
      !this.model.branchConfig['override_approval']
    ) {
      // no override labels configured
      return false;
    }

    for (const requiredApprovalInfo of this.model.branchConfig[
      'override_approval'
    ]) {
  ...
  private getStatusCount() {
    return (this.model?.status?.rawStatuses ?? []).reduce(
      (prev, cur) => {
