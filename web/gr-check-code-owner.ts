...
export class GrCheckCodeOwner extends LitElement {
    ...
    return html`
      <main class="gr-form-styles read-only">
        ...
        <fieldset>
          ...
          global capability.
          <br/><br/>
          All fields, except the 'Calling User' field, are required.
        </fieldset>
        <fieldset>
          <section>
          ...
          <section>
            <span class="title">Calling User (optional):</span>
            <span class="value">
              <input
                id="userInput"
                value="${this.user}"
  ...
  private validateData() {
    this.dataValid =
      ...
      this.validateUser(this.userInput.value) &&
  ...
  private handleCheckCodeOwner() {
    ...
    var url = `/a/projects/${encodeURIComponent(this.project)}/branches/${encodeURIComponent(this.branch)}/code_owners.check/?email=${encodeURIComponent(this.email)}&path=${encodeURIComponent(this.path)}`;
    if (this.user) {
      url = url + `&user=${encodeURIComponent(this.user)}`
    }
    url = url + "&pp=1";
    return this.plugin
      .restApi()
      .get<CheckCodeOwnerInfo>(url)
      .then(response => {
