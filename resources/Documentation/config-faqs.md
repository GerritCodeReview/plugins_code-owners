...
the initial commit are newly [validated](validation.md#codeOwnerConfigValidationOnBranchCreation), even if the branch is created for a
...
1. Use the [code-owners~skip-validation
   validation](validation.md#skipCodeOwnerConfigValidationOnDemand) option to
   skip the validation of code owner config files when creating the branch.
2. Use the
   [Check Code Owner Config Files](rest-api.md#check-code-owner-config-files)
   REST endpoint to validate the code owner files in the new branch (specify
   the branch in the `branches` field in the
   [CheckCodeOwnerConfigFilesInput](rest-api.md#check-code-owner-config-files-input))
...
3. Fix the reported issues and push them as a change for code review. If
   ...
   override](user-guide.md#codeOwnerOverride).
...
creation by [configuration](config.md#pluginCodeOwnersEnableValidationOnBranchCreation).
