...
| `change`    | optional  | Change for which permissions should be checked. If not specified permissions are not checked.
...
| `can_approve_change`| Whether the user to which the given email was resolved can code-owner approve the specified change. Not set if the given email is not resolvable, if the given email is the all users wildcard (aka '*') or if no change was specified.
