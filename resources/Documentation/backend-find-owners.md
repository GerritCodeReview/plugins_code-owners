...
expressions. The path expressions are Java NIO globs and can match absolute
paths or paths relative to the directory of the `OWNERS` file, but they can only
match files in the directory of the `OWNERS` file and its subdirectories.
Multiple path expressions can be specified as a comma-separated list.
