...
* the validation is disabled via the
  ...
  config options
* the [--code-owners~skip-code-owners-validation](#skipCodeOwnerConfigValidationOnDemand)
...
By setting the `--code-owners~skip-code-owners-validation` push option it is
possible to skip the code owner config validation on push.

Using this push option requires the calling user to have to
`Can Skip Code Owner Config Validation` global capability. This also applies to
host admins, since admins do not have this capability by default.
