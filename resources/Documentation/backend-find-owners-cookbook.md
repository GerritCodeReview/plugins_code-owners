...
* files that are matched by a path expression in a
  ...
  in any `OWNERS` file in a subdirectory)
...
approvals](#exemptFiles), assign the code ownership to all users instead.
...
code owner approvals](#exemptFiles), assign the code ownership to all users
instead.
...
[file](backend-find-owners.html#fileKeyword) keyword or the
[include](backend-find-owners.html#includeKeyword) keyword.
